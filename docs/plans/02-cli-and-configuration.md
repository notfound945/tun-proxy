...
`service install`、`upgrade`、`reload` 和 `uninstall` 都按事务处理；普通 uninstall 默认保留
...
托管 reload 会先校验并事务性安装配置，再向服务发送 `SIGHUP`，等待 worker status socket
确认成功或失败；运行时拒绝或超时会回滚已安装配置。reload 成功后只影响新建 DNS 查询和
TCP/UDP 流，已有流保留其原 generation、规则决定和出口。
